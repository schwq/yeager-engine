...
GLuint Yeager::LoadTextureFromData(STBIDataOutput* output)
{
  ...
  if (output->NrComponents == 1)
    format = GL_RED;
  else if (output->NrComponents == 3)
    format = GL_RGB;
  else if (output->NrComponents == 4)
    format = GL_RGBA;

  glTexImage2D(GL_TEXTURE_2D, 0, format, output->Width, output->Height, 0, format, GL_UNSIGNED_BYTE, output->Data);
