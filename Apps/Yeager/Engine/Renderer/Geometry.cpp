...
{
  return std::vector<yg_uint>{0, 1, 3, 3, 1, 2, 1, 5, 2, 2, 5, 6,
                              5, 4, 6, 6, 4, 7, 4, 0, 7, 7, 0, 3,
                              3, 2, 7, 7, 2, 6, 4, 5, 0, 0, 5, 1};
}

Geometry::Geometry(yg_string name, yg_vec3 color, GeometryShape shape,
                   Application* app, bool is_color, Yeager::Texture2D* texture)
    : m_color(color),
      m_shape(shape),
      m_app(app),
      GameEntity(name),
...
Geometry::~Geometry()
{
  ...
  Yeager::Log(INFO, kSystem, "Destrorying geometry shape name: {} ID: {}",
              m_name, m_id);
}
...
void Geometry::GenerateVerticesIndices()
{
  switch (m_shape) {
    case kCube:
      ...
      break;
    default:
      Yeager::Log(ERROR, kSystem, "Invalid geometry shape! Name {} Id {}",
                  m_name, m_id);
  }
}