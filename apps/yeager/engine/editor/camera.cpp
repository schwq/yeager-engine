...
EditorCamera::EditorCamera(Application *app, Vector3 cameraPosition, Vector3 cameraFront,
                           Vector3 cameraUp)
    : m_app(app) {
...
void EditorCamera::MouseCallback(bool &firstMouse, float &lastX, float &lastY, double xpos,
                                 double ypos) {

    if (m_cameraShouldMove && m_app->GetCurrentMode() == ApplicationCurrentMode::kEditorMode) {
        if (firstMouse) {
