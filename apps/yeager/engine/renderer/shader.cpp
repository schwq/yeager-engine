...
Shader::Shader(const char *fragmentPath, const char *vertexPath, String name) : m_name(name) {
...
uint Shader::CreateVertexGL(const char *vertexPath) {
    ...
    if (vertexFile.is_open()) {
        ...
        if (!vertexShaderSuccess) {
            ...
            VLOG_F(ERROR, "Cannot create vertex shader: %s, ID: %u, Error: %s", m_name.c_str(),
                   m_id, vertexInfoLog);
        } else {
            VLOG_F(INFO, "Success in creating shader %s, ID %u", m_name.c_str(), m_id);
        }
...
uint Shader::CreateFragmentGL(const char *fragmentPath) {
    ...
    if (fragmentFile.is_open()) {
        ...
        if (!fragmentShaderSuccess) {
            ...
            VLOG_F(ERROR, "Cannot create fragment shader: %s, ID: %u, Error: %s", m_name.c_str(),
                   m_id, fragmentInfoLog);
        } else {
            VLOG_F(INFO, "Success in creating shader %s, ID %u", m_name.c_str(), m_id);
        }
...
void Shader::LinkShaders(uint vertexShader, uint fragmentShader) {
    ...
    if (!linkSuccess) {
        ...
        VLOG_F(ERROR, "Cannot link shaders: %s, ID: %u, Error: %s", m_name.c_str(), m_id, linkInfo);
    } else {
        VLOG_F(INFO, "Success in linking shaders: %s, ID: %u", m_name.c_str(), m_id);
    }
